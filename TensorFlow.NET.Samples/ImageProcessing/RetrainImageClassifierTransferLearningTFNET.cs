...
namespace TensorFlowNET.Examples.ImageProcess
{
    ...
    /// </summary>
    public class RetrainImageClassifierTransferLearningTFNET : IExample
    {
        ...
        public string Name => "Retrain Image Classifier with TransferLearning in TensorFlow.NET";

        const string root_image_processing = "ImageProcessing";
        const string training_images_dir = "ImageProcessing/ImagesForTraining";
        const string transfer_learning_assets_dir = "ImageProcessing/TransferLearningAssets";
        ...
        string output_labels = Path.Join(transfer_learning_assets_dir, "tfdotnet_my_custom_labels_list.txt");
        // The location where variable checkpoints will be stored.
        string CHECKPOINT_NAME = Path.Join(training_images_dir, "_retrain_checkpoint");
            ...
            Tensor resized_image_tensor, Tensor bottleneck_tensor)
        {
            var (test_bottlenecks, test_ground_truth, test_filenames) = get_random_cached_bottlenecks(train_session, image_lists,
                                    test_batch_size, "testing", bottleneck_dir, images_folder_for_training, jpeg_data_tensor,
        ...
        public void PrepareData()
        {
            ...
            Web.Download(url, training_images_dir, "tfhub_modules.zip");
            Compress.UnZip(Path.Join(training_images_dir, "tfhub_modules.zip"), "tfhub_modules");
            ...
            if (class_count == 0)
                print($"No valid folders of images found at {images_folder_for_training}");
            if (class_count == 1)
                print("Only one valid folder of images found at " +
                     images_folder_for_training +
        ...
        {
            var sub_dirs = tf.gfile.Walk(images_folder_for_training)
                .Select(x => x.Item1)
                .OrderBy(x => x)
                .ToArray();
        ...
        public void Train(Session sess)
        {
            ...
            // cached them on disk.
            cache_bottlenecks(sess, image_lists, images_folder_for_training,
            ...
            for (int i = 0; i < how_many_training_steps; i++)
            {
                var (train_bottlenecks, train_ground_truth, _) = get_random_cached_bottlenecks(
                     ...
                     bottleneck_dir, images_folder_for_training, jpeg_data_tensor,
        ...
        public void Predict(Session sess_)
        {
            ...
            // predict image
            var img_path = Path.Join(images_folder_for_predicting, image_filename_to_use_for_prediction); 
            ...
            with(tf.Session(graph), sess =>
            {
                ...
                print($"Prediction result: [{labels[idx]} {prob[idx][0]}] for {image_filename_to_use_for_prediction}.");
            });
        }
